...
class Discord_Info(commands.Cog):
    ...
    async def vc(self, ctx, channel_name:str):
        try:
            ID = discord.utils.get(ctx.guild.voice_channels, name=channel_name).id
            await ctx.reply(embed = discord.Embed(title = 'Click to join️', description = f'<#{ID}>', color = discord.Color.green()))
        except AttributeError:
            await ctx.reply(embed = discord.Embed(title = 'Voice Channel does not Exist.', description = 'Perhaps check for a spelling mistake', color = discord.Color.dark_red()))
